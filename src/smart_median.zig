...
fn SmartMedian(comptime T: type) type {
    ...
    return struct {
        ...
        fn processPlaneVector(radius: comptime_int, threshold: T, noalias srcp: []const T, noalias dstp: []T, width: usize, height: usize, stride: usize) void {
            ...
            for (radius..height - radius) |row| {
                ...
                for (column..interior_end) |tail_column| {
                    ...
                    dstp[(row * stride) + tail_column] = smartMedianScalar(threshold, &grid);
                }
        ...
        test "vector SmartMedian matches scalar on strided tail widths" {
            const widths = [_]usize{ vector_len, vector_len + 1, vector_len * 2 + 3, vector_len * 2 + 5 };

            inline for ([_]comptime_int{ 1, 2 }) |radius| {
                for (widths) |width| {
                    ...
                    const threshold: T = switch (comptime types.numberType(T)) {
                        .int => @intCast(32),
                        .float => @floatCast(0.25),
                    };
                    processPlaneScalar(radius, threshold, srcp, scalar, width, height, stride);
                    processPlaneVector(radius, threshold, srcp, simd, width, height, stride);
